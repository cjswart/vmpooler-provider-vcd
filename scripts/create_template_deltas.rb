...
def create_template_deltas( folder )
  ...
  vim = RbVmomi::VIM.connect(
    :host     => config[ :providers ][ :vcd ][ "server" ],
    :user     => config[ :providers ][ :vcd ][ "username" ],
    :password => config[ :providers ][ :vcd ][ "password" ],
    :ssl      => true,
    :insecure => true,
  ) or abort "Unable to connect to #{config[ :vcd ][ "server" ]}!"
